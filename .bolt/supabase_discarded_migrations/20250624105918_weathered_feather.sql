/*
  # Create talent discovery infrastructure

  1. New Tables
    - `smart_pools` - For saving search criteria and creating talent pools

  2. Functions
    - `increment_cv_recruiter_view` - Safely increment CV recruiter views
    
  3. Views
    - `candidate_cv_profiles` - Anonymized candidate profiles for recruiters

  4. Security
    - Enable RLS on all new tables and views
    - Add policies for recruiter access to talent discovery features
*/
...
SELECT 
  ...
  (
    SELECT string_agg(DISTINCT e.degree || ' in ' || COALESCE(e.field_of_study, 'General'), ', ')
    FROM education e 
    WHERE e.cv_id = c.id
    LIMIT 3
